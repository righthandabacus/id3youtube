...
# GNU DBM for memoization
DBFILE = 'memo.db'

class dbmwrapper:
    '''A wrapper to DBM, which the open and close of file is hidden
    Usage:
        Write:
            b = dbmwrapper(filename)
            b.put(key, value)
        Read:
            b = dbmwrapper(filename)
            cur = b.cursor()
...
def memoize(wrapper):
    '''memoization decorator to use DBM

    Args:
        db: Instance of dbmwrapper object
    '''
    def _deco(func):
        def _func(*arg, **kwargs):
            key = pickle.dumps([func.__qualname__, arg, kwargs])
            try:
                val = pickle.loads(wrapper.db[key])
            except KeyError:
                val = func(*arg, **kwargs)
                wrapper.db[key] = pickle.dumps(val)
            return val
        return _func
    return _deco

wrapper = dbmwrapper(DBFILE)
